...
Each workflow uploads only its own JARs and SHA-256 files, then publishes that loader to Modrinth and CurseForge. To repair an existing release, select **Run workflow** in the matching workflow and enter the Release tag, destinations, and an optional version filter. Fabric filters use `1.21.x`, `26.1.x`, `26.2`, or `26.3`; NeoForge filters use concrete targets such as `1.21.1`, `26.1.2`, or `26.3`. Configure `MODRINTH_TOKEN` and `CURSEFORGE_TOKEN` as repository secrets; project IDs can be overridden with Repository Variables.
